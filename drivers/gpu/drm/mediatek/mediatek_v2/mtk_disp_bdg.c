...
static void bdg_config_init(struct device_node *node)
{
	...
	ret = of_property_read_u32(node, "is_ext", &is_ext);
	if (!ret) {
		DDPINFO("%s: read is_ext:%d,", __func__, is_ext);
	} else {
		is_ext = 0;
		DDPINFO("%s: not find is_ext, set as: %d!", __func__, bdg_rx_v12);
	}
