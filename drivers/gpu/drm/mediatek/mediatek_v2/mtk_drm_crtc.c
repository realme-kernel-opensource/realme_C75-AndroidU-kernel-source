...
extern bool underflow_reset_flag;
...
int mtk_drm_aod_setbacklight(struct drm_crtc *crtc, unsigned int level)
{
	...
	if (output_comp->funcs && output_comp->funcs->io_cmd) {
		if (is_bdg_supported() && mtk_crtc && mtk_crtc->panel_ext && mtk_crtc->panel_ext->params &&
				mtk_crtc->panel_ext->params->oplus_display_lcd_6382_aod == 1) {
			output_comp->funcs->io_cmd(output_comp, cmdq_handle, DSI_STOP_VDO_MODE, NULL);
			output_comp->funcs->io_cmd(output_comp, cmdq_handle, DSI_SET_BL_AOD, &level);
			output_comp->funcs->io_cmd(output_comp, cmdq_handle, DSI_START_VDO_MODE, NULL);
			mtk_disp_mutex_trigger(mtk_crtc->mutex[0], cmdq_handle);
			output_comp->funcs->io_cmd(output_comp, cmdq_handle, COMP_REG_START, NULL);
		}
		else
			output_comp->funcs->io_cmd(output_comp, cmdq_handle, DSI_SET_BL_AOD, &level);
	}
...
#ifndef DRM_CMDQ_DISABLE
	if (underflow_reset_flag && priv && (priv->data->mmsys_id == MMSYS_MT6768)) {
		underflow_reset_flag = false;
		mtk_ddp_sw_reset(state->cmdq_handle, mtk_crtc);
	}
#endif
