...
enum {
	...
	BAT_TYPE__ATL_4470mV, //70mv~180mv
	BAT_TYPE__ATL_4510mV, //250mv~350mv
};
...
int battery_type_check(int *battery_type)
{
	...
	if (is_batt_id_check()) {
		...
		if(is_fuelgauge_apply() == true){
			if (use_4510mv_battery) {
				if ((value >= 200 && value <= 350) || (value >= 550 && value <= 820)) {
					*battery_type = BAT_TYPE__ATL_4510mV;
					battery_id = 0;
				} else {
					*battery_type = BAT_TYPE__UNKNOWN;
					battery_id = 0;
				...
				if (value >= 790 && value <= 1100) {
					...
					*battery_type = BAT_TYPE__ATL_4470mV;
					battery_id = 0;
				} else {
					*battery_type = BAT_TYPE__UNKNOWN;
					battery_id = 0;
...
try_again:
	...
	if (battery_type == BAT_TYPE__ATL_4450mV || battery_type == BAT_TYPE__XWD_4450mV || battery_type == BAT_TYPE__XWD_4480mV || battery_type == BAT_TYPE__ATL_4510mV) {
		return true;
	} else {
		if (retry_flag == 0) {
			retry_flag = 1;
...
int battery_init(struct platform_device *pdev)
{
	...
	fg_read_dts_val(pdev->dev.of_node, "USE_4510MV_BATTERY", &(use_4510mv_battery), 1);
	bm_err("%s, USE_4510MV_BATTERY:%d\n", __func__, use_4510mv_battery);
