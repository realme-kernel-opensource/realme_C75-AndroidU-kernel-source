...
static int get_hw_AP_temp(void)
{
	...
	if (!thermistor_ch0_1) {
		mtkts_AP_printk("thermistor_ch0_1 NULL\n");
		return -1;
	}
	ret = iio_read_channel_processed(thermistor_ch0_1, &val);
	//val = get_bb_ntc_volt();
	mtkts_AP_printk("%s val=%d\n", __func__, val);

	if (ret < 0) {
		mtkts_AP_printk("IIO channel read failed %d\n", ret);
		return ret;
	}
	/* ret = ret*1800/4096; 82's ADC power */
	mtkts_AP_printk("APtery output mV = %d\n", val);
	output = mtk_ts_AP_volt_to_temp(val);
	mtkts_AP_printk("AP output temperature = %d\n", output);
	return output;
}
...
static struct thermal_zone_device_ops mtk_AP_dev_ops = {
	...
	.change_mode = mtk_AP_change_mode,
