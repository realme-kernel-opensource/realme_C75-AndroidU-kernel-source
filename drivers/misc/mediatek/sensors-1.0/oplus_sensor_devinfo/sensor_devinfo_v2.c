...
#endif
		bdev = blkdev_get_by_path(OPLUSCUSTOM_FILE, FMODE_READ | FMODE_WRITE, THIS_MODULE);
		if (!IS_ERR(bdev)) {
			pr_err("%s:Sylvia success to get dev block!!!\n", __func__);
			return bdev;
		}
/*Sylvia mod end*/
...
EXPORT_SYMBOL_GPL(oplus_send_factory_mode_cmd_to_hub);
...
EXPORT_SYMBOL_GPL(oplus_send_selftest_cmd_to_hub);
